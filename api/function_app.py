...
def tract_id_human(tract_str: str) -> str:
    """Convert '340106' -> '3401.06' style for display."""
    if not tract_str:
        return ""
    t = tract_str.zfill(6)
    return f"{t[:4]}.{t[4:]}"
...
def get_market_stats_for_zip(zip_code: str) -> Dict[str, Optional[int]]:
    ...
    try:
        ...
        if resp.status_code == 404:
            logging.warning("RapidAPI 404 for ZIP %s", zip_code)
            _dom_cache[zip_code] = None
            return {"median_days_on_market": None}
        ...
        for p in props:
            dom = (p.get("days_on_market") or
                   p.get("list_days_on_market") or
                   p.get("dom"))
...
def get_zip_for_tract(county_fips: str, tract: str) -> Optional[str]:
    ...
    if county_fips == "097":  # Marion (Indianapolis)
        ...
        if head <= 15:
            ...
            return "46218"
    if county_fips == "095":  # Madison (Anderson)
        return "46016"
...
# -------------------------------------------------
# Aggregation helpers (population-weighted)
# -------------------------------------------------
